...
class AttendanceCheck:
    ...
    def attendance_check_in(self) -> attendance_check_type:
        ...
        current_time = datetime.strptime(self.get_current_time(self.coordinate.latitude, self.coordinate.longitude),
                                         "%H:%M:%S", )
        ...
        if not self.coordinate_validation_strategy.is_within_error_range(company_coordinates, self.coordinate):
    ...
    def attendance_check_out(self) -> attendance_check_type:
        # 1. account_id 정보가 DB에 존재하는지 확인
        account_info = fake_db.get_account(self.account_id)
        if not account_info:
            raise ValueError("유효한 계정이 아닙니다.")
        ...
        if not self.coordinate_validation_strategy.is_within_error_range(company_coordinates, self.coordinate):
            ...
            # 출근 기록 저장
            fake_db.save_check_out(self.account_id, current_time)
